...
def download_to_file(remote, local, mode='wb', debugfnc=None):
	...
	try:
		...
		try:
			...
			f = urlopen(remote, context = context, timeout=10)
		except Exception:
			f = urlopen(remote, timeout = 10)
...
class CustomImporter:
	...
	def add_path(self, path):
		if not path in self.__path:
			self.log("%s add path '%s'" , self, path)
			self.__path.append(path)

	def add_module(self, mod_name, mod):
		self.log("%s add module '%s':'%s'" , (self, mod_name, mod))
	...
	def find_module(self, fullname, path):
		...
		try:
			...
			self.log("%s found module '%s' <filename:%s description:%s>" , self, fullname, self.filename, self.description)
		except ImportError:
	...
	def find_spec(self, fullname, path, target=None):
		self.log('%s import %s from %s' % (self, fullname, path))
		
		if fullname in self.__specs:
			self.log('%s found %s in specs' % (self, fullname) )
			self.__spec_name = self.__specs[fullname].name
			return self.__specs[fullname]
		
		if fullname in self.__modules and fullname not in self.__specs:
			self.log('%s creating new spec for module %s' % (self, fullname) )
			spec = machinery.ModuleSpec(fullname, self)
			self.__spec_name = spec.name
			self.__specs[fullname] = spec
			return spec
		...
		try:
			...
			if spec is None:
				self.log('%s module %s not found.' % (self, fullname) )
				return None
			
			spec.loader = self
			self.__spec_name = spec.name
			self.__specs[fullname] = spec
			self.log('%s found module %s' % (self, spec.name))
			return spec
		except:
			self.log("%s import error\n%s" % (self, traceback.format_exc()))
	...
	def create_module(self, spec):
		self.log('%s creating module %s' % (self, spec) )
		
		if spec.name in self.__modules:
			return self.__modules[spec.name]
		return ModuleType(spec.name)

	# importlib implementation
	def exec_module(self, module):
		self.log('%s executing module %s' % (self, module))
		spec_name = self.__spec_name
		if spec_name in self.__modules:
			return self.__modules[spec_name]
		
		if spec_name not in self.__specs:
			return None
		
		code = None
		with open(self.__specs[spec_name].origin, 'rb') as f:
			code = f.read()
			
		mod = self.__modules[spec_name] = module
		mod.__file__ = self.__specs[spec_name].origin
		mod.__loader__ = self
		del self.__spec_name
		
		try:
			if os.path.splitext(self.__specs[spec_name].origin)[1] in ['.pyo', '.pyc']:
				code_bytes = code[8:]
				code = marshal.loads(code_bytes)
				self.log('%s  exec_module() code type=%s' % (self, type(code)))
			else:
				code = compile( code, mod.__file__, 'exec')
				
			exec( code,  mod.__dict__ )
			self.log('%s module %s executed' % (self, spec_name) )
		except Exception:
			self.log("%s module %s %s exec error:\n%s" % (self, self.name, spec_name, traceback.format_exc()))
			del self.__modules[spec_name]
			raise
