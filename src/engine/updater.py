...
class ArchivUpdater(object):
	def __init__(self, archivInstance):
		...
		self.updateXml = "http://cdn.rawgit.com/%s/archivczsk/{commit}/build/plugin/update/app.xml" % github_params[0]
		self.updateZip = "http://cdn.rawgit.com/%s/archivczsk/{commit}/build/plugin/update/version/archivczsk-{version}.zip" % github_params[0]
		self.commit = "https://raw.githubusercontent.com/%s/archivczsk/%s/build/plugin/update/commit" % github_params
